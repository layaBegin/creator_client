...
export default class PeekCard extends cc.Component {
    ...
    _touchMove(event?: cc.Event.EventTouch) {
        ...
        if (this._isMoveStart == false && length >= this._directionLength) {
            ...
            //移动
            this._moveByVec2(this.mask.node, unitV, this._moveSpeed);
            this._moveByVec2(this.cardBack, _unitV, this._moveSpeed);
            this._moveByVec2(this.cardFace, unitV, this._moveSpeed);
            this._moveByVec2(this.shadow, _unitV, this._moveSpeed);
            //显示手指
    ...
    async _loadSpriteFrameSync(url: string): Promise<cc.SpriteFrame> {
        ...
        return new Promise<cc.SpriteFrame>((resolve) => {
            AssetMgr.loadResSync(url, cc.SpriteFrame, undefined, (err, res: cc.SpriteFrame) => {
                resolve(res)
            });
        })
    }
    ...
    async _setNodeSpriteFrame(node: cc.Node, spf: string | cc.SpriteFrame) {
        ...
        cc.isValid(node) && (node.getComponent(cc.Sprite).spriteFrame = spf);
        return spf;
    }
}
