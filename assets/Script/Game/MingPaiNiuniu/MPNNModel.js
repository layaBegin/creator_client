...
model.setEntryRoomData = function (msg) {
	// cc.log("=======明牌牛牛进房数据",JSON.stringify(msg,null,4));
	/*
	=======明牌牛牛进房数据 {
    "roomUserInfoArr": [
        {
            "userInfo": {
                "uid": "100001",
                "nickname": "1557544574897",
                "avatar": "UserInfo/head_7",
                "gold": 99986164.91,
                "permission": 1,
                "spreaderID": "",
                "sex": 0,
	...
	*/
	...
	if (this.gameStatus === MPNNProto.GAME_STATUS_RESOUT) {
	...
	this.firstCardCount = this.gameRule.otherRule.mingcard;
	...
	for (i = 0; i < this.userArr.length; ++i) {
		if (this.userArr[i].userInfo.uid === this.myUid) {
			...
			cc.log("===myChairId:%s,myUid:%s", this.myChairId, this.myUid);
	...
	if (data.gameStatus === 2 || data.gameStatus === 3) {
		this.cardArr = data.cardsArr || [];
	} else {
		if (!!data.cardsArr) {
			let myIndex = this.getChairIdIndex(this.myChairId);
...
model.messageCallbackHandler = function (router, msg) {
	if (router === 'RoomMessagePush') {
		...
		} else if (msg.type === MPNNProto.FOUR_CARD_PUSH) {
...
// model.answerRoomUserInfoChangePush = function(data){
...
model.answerFourCardPush = function (cardArr) {
...
model.answerGameStatusPush = function (gameStatus) {
	...
	if (this.gameStatus === MPNNProto.GAME_STATUS_ROBBANK) {
		this.gameRule.memberCount = this.gameStartChairIdArr.length;
		for (i = 0; i < this.gameRule.memberCount; ++i) {
			this.robBankArr[i] = -1;
		}
		this.recordGameStartChairId();
	} else if (this.gameStatus === MPNNProto.GAME_STATUS_POURSCORE) {
		this.pourScoreArr = [];
		for (i = 0; i < this.gameRule.memberCount; ++i) {
			this.pourScoreArr[i] = 0;
		}
	} else if (this.gameStatus === MPNNProto.GAME_STATUS_SORTCARD) {
		this.cardsArr = [];
		for (i = 0; i < this.gameRule.memberCount; ++i) {
			this.cardsArr[i] = [];
		}
...
model.getFirstCardCount = function () {
...
model.getPlayerByUid = function (uid) {
	for (var i = 0; i < this.userArr.length; ++i) {
