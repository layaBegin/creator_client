...
gameLogic.getSpecialCardTypeRate = function(cards) {
	var rate = 0;
	if(this.isFourSameCard(cards)) {
		rate = gameLogic.RATE_ZHADAN;
	}
	else if(this.isFiveColorNiu(cards)) {
 ...
 * @param twoGroup
 */
gameLogic.getCardTypeBy2Group = function (oneGroup, twoGroup) {
	if (!oneGroup || !twoGroup || oneGroup.length!=3 || twoGroup.length!=2) return 0;

	var sum = 0;
	for (var i = 0; i < oneGroup.length; ++i) {
		sum += this.getCardCount(oneGroup[i]);
	}
	if (sum%10 != 0) return 0;
	sum = 0;
	for (i = 0; i < twoGroup.length; ++i) {
		sum += this.getCardCount(twoGroup[i]);;
	}
	if (sum > 10) {
		sum -= 10;
	}
...
//四花牛
gameLogic.isFourColorNiu = function (cards) {
	var i;
	var m = 0;
	var n = 0;
	for (i = 0; i < cards.length; ++i) {
...
//五小牛
gameLogic.isFiveXiaoNiu = function (cards) {

	var cardLogicValue = [];
	var totalLogicSum = 0;
	for (var i = 0; i < cards.length; ++i) {
		cardLogicValue[i] = this.getCardCount(cards[i]);
		totalLogicSum += cardLogicValue[i];
	}
	if (totalLogicSum >= 10)
		return false;
	for (var i = 0; i < cardLogicValue.length; ++i) {
		if (cardLogicValue[i] >= 5) {
			return false;
		}
	}
	return true;
...
//获得排序手牌 ，cardData参数为[]
gameLogic.getSortCard = function (cardData) {
	let sortArr = [];
	if (cardData instanceof Array == false) {
		return ;
	}
	var cardLogicValue = [];
	var totalLogicSum = 0;
	for (var i = 0; i < cardData.length; ++i) {
		cardLogicValue[i] = this.getCardCount(cardData[i]);
		totalLogicSum += cardLogicValue[i];
	}
	for (var i = 0; i < cardData.length - 1; i++) {
		for (var j = i + 1; j < cardData.length; j++) {

			if ((totalLogicSum - cardLogicValue[i] - cardLogicValue[j]) % 10 == 0) {
				for (var index = 0; index < cardData.length; ++index) {
					if (index !== i && index !==j){
						sortArr.push(cardData[index])					}
				}
				sortArr.push(cardData[i]);
				sortArr.push(cardData[j]);
				return sortArr;
			}
		}
	}
