...
proto.GAME_OPERATION_PLAYER_PUSH = 411;		// 当前操作玩家通知
...
proto.gameOperationPlayerPush = function (curtplayer, index, time) {
  return {
    type: this.GAME_OPERATION_PLAYER_PUSH,
    data: {
      curtplayer: curtplayer,
      cardindex: index,
      worktime: time
    }
  };
};
proto.gameStartPush = function (playingStatusArr, time) {
  return {
...
proto.gameSendCardPush = function (bankerCardArr, userCardArr, time) {
  return {
...
// 游戏结果推送
proto.gameResultPush = function (bankerCardArr, allCardArr, scoreChangeArr, time, profitPercentage) {
  return {
