...
proto.CALLBANKARR_CHANGE_PUSH = 413; // 随庄推送
...
proto.MAX_CALL_BANKER_PUSH = 412 //叫庄最大倍数推送
...
// 随庄推送
proto.getBankArrChangePushData = function (bankerarr) {
	return {
		type: this.CALLBANKARR_CHANGE_PUSH,
		data: {
			bankerarr: bankerarr
		}
	};
...
// 选择押注分数推送
proto.getCanPourScorePushData = function (gameStatus, addscoresArr) {
	return {
		type: this.CAN_POUR_SCORE_PUSH,
		data: {
			gameStatus: gameStatus,
			addscoresArr: addscoresArr
		}
	};
};
...
// 叫庄倍数推送
proto.getcallbankerPushData = function (maxcallbanker) {
	return {
		type: this.MAX_CALL_BANKER_PUSH,
		data: {
			Maxcallbanker: maxcallbanker
		}
	};
};
// 亮牌通知
...
proto.getGameResoutPushData = function (resout) {
	return {
		...
		data: {
			...
			gameStartChairIdArr: resout.gameStartChairIdArr,
			profitPercentage: resout.profitPercentage
		}
	};
};

// 游戏状态变化推送
proto.getGameStatusPushData = function (gameStatus, time) {
	return {
