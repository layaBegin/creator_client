...
cc.Class({
    ...
    properties: {
        ...
        lockLineWidgetCtrl: require("ShenShouTianXiaLockLineWidgetControl"),
    ...
    initWidget(roomUserInfo, isSelf, posIndex, winGold, changeCannonPowerCallback) {
        ...
        let cannonBackUrl = "ShenShouTianXia/cannon/deco" + tempValue;
    ...
    updateCannonPic: function () {
        let gunUrl = "ShenShouTianXia/cannon/gun";
        if (this.isSuper === true) {
            gunUrl = "ShenShouTianXia/cannon/gun_super_";
        }
        let gunIndex = 1;
        if (this.powerIndex <= 3) {
            gunIndex = 1;
        } else if (this.powerIndex <= 5) {
            gunIndex = 2;
        } else if (this.powerIndex <= 8) {
            gunIndex = 3;
        } else {
            gunIndex = 4;
        }
        gunUrl = gunUrl + gunIndex;
        let cannonSpr = this.cannon.getComponent(cc.Sprite);
        Global.CCHelper.updateSpriteFrame(gunUrl, cannonSpr);
    },
    ...
    onLockFish: function (fishCtrl) {
        if (!fishCtrl || !fishCtrl.node) {
            ...
            let lockFishCtrl = this.lockFish.getComponent("ShenShouTianXiaFishCtrl");
    ...
    onBtnClick: function (event, params) {
        if (params === "addPower") {
            ...
            AudioMgr.playSound('ShenShouTianXia/sound/effect/cannonSwitch');
    ...
    cannonFireEffect() {
        ...
        if (this.powerIndex <= 3) {
            this.fireEffect1.active = true;
            fireEffectCtrl = this.fireEffect1.getComponent("SpriteFrameAnimationWidgetCtrl");
        } else if (this.powerIndex <= 5) {
            this.fireEffect2.active = true;
            fireEffectCtrl = this.fireEffect2.getComponent("SpriteFrameAnimationWidgetCtrl");
        } else {
            this.fireEffect3.active = true;
            fireEffectCtrl = this.fireEffect3.getComponent("SpriteFrameAnimationWidgetCtrl");
        }
        fireEffectCtrl.initAnimation();
        fireEffectCtrl.startAnimation(false, 1, function () {
            fireEffectCtrl.node.active = false;
        });
    },
    ...
    showPrize(gainGold, baseValue) {
        ...
        if (rewardTimes < 50) {
            ...
            AudioMgr.playSound('ShenShouTianXia/sound/effect/bingo');
            return;
        }
        if (rewardTimes < 100) {
        ...
        if (!!aniCtrl) {
            ...
            AudioMgr.playSound('ShenShouTianXia/sound/effect/prize1');
        }
    },
    ...
    showGoldPillar(goldValue, baseValue) {
        ...
        for (let i = 0; i < len; ++i) {
            let pillarctrl = this.pillarList[i].getComponent("ShenShouTianXiaGoldPillarControl");
            if (i == 0 && len == 4) {
        ...
        let pillarCtrl = pillarItem.getComponent("ShenShouTianXiaGoldPillarControl");
        let showTime = 0.15;
        let moveDistance = 33;
        let moveDir = 1;
