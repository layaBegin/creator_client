...
cc.Class({
    ...
    initWidget: function (chairID, bulletId, bulletLevel, superBullet, captureCallback) {
        ...
        if (this.bulletLevel <= 3) {
            ...
            this.node.width = 40;
            this.node.height = 58;
            this.boxCollider.size.width = 40;
            this.boxCollider.size.height = 58;
        } else if (this.bulletLevel <= 7) {
            powerLevel = 2;
            this.node.width = 75;
            this.node.height = 77;
            this.boxCollider.size.width = 75;
            this.boxCollider.size.height = 77;
        } else {
            powerLevel = 3;
            this.node.width = 85;
            this.node.height = 82;
            this.boxCollider.size.width = 85;
            this.boxCollider.size.height = 82;
        }

        let res = cc.loader.getRes("ShenHaiBuYu/Atlas/bullet", cc.SpriteAtlas);
        let bulletName = "";
        if (this.superBullet === true) {
            bulletName = "bullet" + powerLevel + "_1";
        } else {
            bulletName = "bullet" + powerLevel + "_1";
        }
    ...
    onCollisionEnter: function (other) {
        ...
        if (type === "wall") {
            ...
            let tempDistance = 3000;
            let moveBy = new cc.Vec2(this.unitVector.x * tempDistance, this.unitVector.y * tempDistance);
            this.node.stopAllActions();
            this.node.runAction(cc.moveBy(tempDistance / BULLET_MOVE_SPEED, moveBy));
            // 修改方向
            ...
            let fishCtrl = other.node.getComponent("ShenHaiBuYuFishCtrl");
            if (!fishCtrl) return;
            // 过滤鱼
