...
let gameProto = require('./API/ShenHaiBuYuProto');
...
cc.Class({
    ...
    properties: {
        cannonBaseArr: [cc.Node],//炮台背景
        cannonPosArr: [cc.Node],//炮台位置
        cannonWidgetPrefab: cc.Prefab,//炮台预制体
        //开火节点
        fireControlWidgetCtrl: require("ShenHaiBuYuFireCtrl"),
        //特效节点
        effectCtrl: require("ShenHaiBuYuEffectCtrl"),
        //撒网节点
        bulletRoot: cc.Node,
        sprayRoot: cc.Node,//水动画层
        bulletWidgetPrefab: cc.Prefab,//子弹

        bulletBoom0: cc.Node,//撒网1
        bulletBoom1: cc.Node,//撒网2
        bulletBoom2: cc.Node,//撒网3

        fishRoot: cc.Node,//鱼的根节点
        fishWidgetPrefab: cc.Prefab,//鱼的组合，大三元，大四喜

        rightMenuRoot: cc.Node,//右菜单，锁定，自动，冰冻
        menuOutBtnNode: cc.Node,//
        menuInBtnNode: cc.Node,//
        exitBtnNode: cc.Node,//
        helpBtnNode: cc.Node,//
        musicBtnNode: cc.Node,
        effectBtnNode: cc.Node,//音效

        toggleAutoFire: cc.Toggle,//自动开火按钮
        toggleLockFish: cc.Toggle,//锁定按钮
        speedBtnNode: cc.Node,//加速按钮
        fixBtnNode: cc.Node,//冰冻按钮
        fixTimeLabel: cc.Label,//冰冻时间
        speedTimeLabel: cc.Label,//加速时间Label

        lightningPrefab: cc.Prefab,//灯光预制
        sprayPrefab: cc.Prefab,//于潮来袭预制

        backBottom: cc.Node,//
        backTop: cc.Node,//全背景

        backItem: cc.Node,//背景
        boomEffect: cc.Node,//爆炸特效
        ...
        wallRoot: cc.Node,//四面墙壁

        waterMarkNode: cc.Node,//水波纹层

        leftNode: cc.Node,//左边节点，包含退出 音效按钮；
        rightNode: cc.Node,//右边节点，包含特效按钮

        modeSpr: cc.Node,//雾罩
        fixMask: cc.Node,//冰冻遮罩
        speedMask: cc.Node,//加速遮罩

        speedHint: cc.Node,//你已经开启急速模式赶紧捕鱼吧
        fixHint: cc.Node,//你已开启冰冻模式，赶紧捕鱼吧

        noFireTip: cc.Node,//长时间未进行游戏，自动退出游戏
        noFireCountLabel: cc.Label,//长时间未开火计时
    },
    ...
    start() {
        ...
        this.bgmList = ["ShenHaiBuYu/sound/bgm/bgm1", "ShenHaiBuYu/sound/bgm/bgm2", "ShenHaiBuYu/sound/bgm/bgm3", "ShenHaiBuYu/sound/bgm/bgm4"];
        ...
        this.modeSpr.stopAllActions();
        this.modeSpr.active = false;
        this.fixMask.active = false;
        this.speedMask.active = false;
        this.selfFireTime = 0;
    },
        ...
        if (router === 'RoomMessagePush') {
            if (msg.type === roomProto.USER_LEAVE_ROOM_PUSH) {
                if (msg.data.roomUserInfo.userInfo.uid === Global.Player.getPy('uid')) {
                    ViewMgr.goBackHall(Config.GameType.SHBY);
                } else {
            ...
            } else if (msg.type === gameProto.GAME_USER_SKILLS_PUSH) {
                this.userSkillsPush(msg.data);
            } else if (msg.type === gameProto.GAME_USER_SKILLS_OK_NOTIFY) {
                this.userSkillsOkNotify(msg.data);
            } else if (msg.type === gameProto.GAME_USER_SKILLS_KILL_NOTIFY) {
                this.userSkillsKillNotify(msg.data);
            }
    ...
    onBtnClick: function (event, params) {
        ...
        if (params === "exit") {
            ...
            event.node.getChildByName("Background").active = !toggle.isChecked;
            ...
            if (toggle.isChecked === true) {
                this.selectedLockFishLeaved(this.selfChairID);
            }
        } else if (params === "speed") {
            this.clickSpeedBtn();
        } else if (params === "fix") {
            if (this.currentState === gameProto.FGStatus.ReadyFishArrayStatus) {
                cc.log("鱼阵状态下无法使用冰冻技能");
                return;
            }
            this.clickFixBtn();
            ...
            if (musicVolume > 0) {
                ...
                musicVolume = 0;
                Global.CCHelper.updateSpriteFrame('ShenHaiBuYu/music_off_btn', musicSpr);
            } else {
                ...
                Global.CCHelper.updateSpriteFrame('ShenHaiBuYu/music_on_btn', musicSpr);
            }
            ...
            if (soundVolume > 0) {
                ...
                soundVolume = 0;
                Global.CCHelper.updateSpriteFrame('ShenHaiBuYu/effect_off_btn', effectSpr);
            } else {
                ...
                Global.CCHelper.updateSpriteFrame('ShenHaiBuYu/effect_on_btn', effectSpr);
            }
            AudioMgr.setSoundVolume(Number(soundVolume));
        } else if (params === "help") {
            Global.DialogManager.createDialog("ShenHaiBuYu/ShenHaiBuYuHelpDialog");
    ...
    gameInit: function (gameData, roomUserInfoArr) {
        ...
        this.fixSkillCdTime = gameData.skillsOkTime1;
        this.speedSkillCdTime = gameData.skillsOkTime2;

        this.currentFixSkillCDAllTime = this.fixSkillCdTime / 2;
        this.currentFixSkillCDTime = this.currentFixSkillCDAllTime;
        this.currentSpeedSkillCDAllTime = this.speedSkillCdTime / 2;
        this.currentSpeedSkillCDTime = this.currentSpeedSkillCDAllTime;

        this.currentFixSkillContinuousTime = null;
        this.currentSpeedSkillContinuousTime = null;
        ...
        if (this.currentState === gameProto.FGStatus.FishArrayStatus) {//鱼阵状态下生成鱼
            ...
            this.scheduleOnce(function () {
                ...
                this.showSnowEffect(tempPos, fixTime);
            }.bind(this), 0.2);
        ...
        let backStr = "ShenHaiBuYu/background/bg" + this.backIndex;
    ...
    fireFailure: function (data) {
        ...
        cannotCtrl.goldChange(returnGold, false);
    },
    ...
    onUserEntry: function (roomUserInfo, curWinGold) {
        ...
        let ctrl = node.getComponent("ShenHaiBuYuCannonCtrl");
        ...
        this.cannonBaseArr[posIndex].active = true;
    },
    ...
    onUserLeave: function (roomUserInfo) {
        ...
        this.cannonBaseArr[posIndex].active = false;
    },
    ...
    onSelfFire: function (targetWorldPoint) {
        ...
        this.playSound('ShenHaiBuYu/sound/effect/fire1', false, 0.3);
    ...
    showSceneBullet: function (bulletData) {
        ...
        let bulletCtrl = node.getComponent('ShenHaiBuYuBulletCtrl');
    ...
    onFire: function (chairID, bulletdate) {
        ...
        this.playSound('ShenHaiBuYu/sound/effect/fire1', false, 0.05);
    ...
    onBeShot: function (bullet, fishCtrl) {
        // 移除子弹
        bullet.onShootFish();
        // this.playSound("ShenHaiBuYu/sound/effect/net2", false, 0.3);
        // 播放爆炸动画
        let boomNode = null;
        if (bullet.bulletLevel <= 3) {
            boomNode = cc.instantiate(this.bulletBoom0);
        } else if (bullet.bulletLevel <= 7) {
            boomNode = cc.instantiate(this.bulletBoom1);
        } else {
            boomNode = cc.instantiate(this.bulletBoom2);
        }
        if (!!boomNode) {
            boomNode.parent = this.bulletRoot;
            boomNode.active = true;
            boomNode.position = bullet.node.position;
            let boomNodeList = boomNode.getChildren();
            if (!!boomNodeList) {
                let len = boomNodeList.length;
                let boomAnimation = null;
                for (let i = 0; i < len; ++i) {
                    boomAnimation = boomNodeList[i].getComponent(cc.Animation);
                    if (!!boomAnimation) {
                        boomAnimation.play();
                        boomAnimation.on("stop", (event) => {
                            if (!!boomNode) {
                                boomNode.destroy();
                            }
                        }, this);
    ...
    onCapture: function (data) {
        ...
        this.playSound('ShenHaiBuYu/sound/effect/coin2', false, 0.3);
    ...
    playDieSound: function (fishTypeID, fishID) {
        if (fishTypeID === fishConfig.FishKind.LocalBomb || fishTypeID === fishConfig.FishKind.FixBomb
            ...
            if (fishID % 2 == 0) {
                this.playSound("ShenHaiBuYu/sound/effect/boom", false, 0.6);
            } else {
                this.playSound("ShenHaiBuYu/sound/effect/superarm", false, 0.6);
            }
            return;
        }
        if (fishTypeID <= fishConfig.FishKind.FishKind10) {
            this.playSound("ShenHaiBuYu/sound/effect/" + (fishID % 2 == 0 ? "hit0" : "hit1"), false, 1);
        }
        else {
            this.playSound("ShenHaiBuYu/sound/effect/" + (fishID % 2 == 0 ? "m" : "f") + (fishID % 7 + 1), false, 1);
        }
    },
    ...
    createLightning: function (startPos, endPos) {
        ...
        let ctrl = node.getComponent("ShenHaiBuYuLighntingWidgetCtrl");
    ...
    //显示定屏炸弹雪花特效
    showSnowEffect: function (pos, fixTime) {
    ...
    addFish: function (fishInfo, fishDelay, isInit, fixTime) {
        ...
        let ctrl = node.getComponent("ShenHaiBuYuFishCtrl");
    ...
    updateState: function (status) {
        // cc.log("[状态切换] status:" + status);
        let fixScreenValue = status & gameProto.FGStatus.FixScreen;
        // cc.log("[状态切换] fixScreenValue:" + fixScreenValue);
        if (fixScreenValue === gameProto.FGStatus.FixScreen) {
            // cc.log("[状态切换] 定屏");
            this.fixScreen(true);
        }
        let fishArrValue = status & gameProto.FGStatus.FishArrayStatus;
        // cc.log("[状态切换] fishArrValue:" + fishArrValue);
        if (fishArrValue === gameProto.FGStatus.FishArrayStatus) {
            cc.log("[状态切换] 鱼阵");
        }
        let readyFishArrValue = status & gameProto.FGStatus.ReadyFishArrayStatus;
        // cc.log("[状态切换] readyFishArrValue:" + readyFishArrValue);
        if (readyFishArrValue === gameProto.FGStatus.ReadyFishArrayStatus) {
            // cc.log("[状态切换] 鱼阵准备");
            if (!this.bgmIndex) {
                this.bgmIndex = 0;
            }
            ...
            AudioMgr.startPlayBgMusic(this.bgmList[this.bgmIndex]);
            // Global.AudioManager.startPlayBgMusic(this.bgmList[this.bgmIndex]);
        ...
        fixScreenValue = this.currentState & gameProto.FGStatus.FixScreen;
        // cc.log("[状态切换]当前状态是否定屏:" + fixScreenValue);
        if (fixScreenValue === gameProto.FGStatus.FixScreen) {
            // let normalStateValue = status & gameProto.FGStatus.Normal;
            // cc.log("[状态切换]更新状态是否正常:" + normalStateValue);
            // if (normalStateValue === gameProto.FGStatus.Normal) {//恢复定屏状态
            // cc.log("[状态切换]定屏恢复正常状态");
    ...
    showSpray() {
        ...
        sprayNode.parent = this.sprayRoot;
        let ctrl = sprayNode.getComponent("ShenHaiBuYuSprayWidgetCtrl");
        ctrl.setInfo(false, sprayActionTime);
        this.playSound("ShenHaiBuYu/sound/effect/wave", false, 0.5);
    },
    ...
    fixScreen: function (isFixScreen) {
        ...
        if (isFixScreen === true) {
            let windowSize = cc.view.getVisibleSize();
            this.showSnowEffect(cc.v2(0, windowSize.height / 2));
        }
    },
    ...
    updateFixSkillTime: function (dt) {
        let tempFixSkillTime = 0;
        if (!!this.currentFixSkillContinuousTime) {
            if (this.currentFixSkillContinuousTime > 0) {
                this.currentFixSkillContinuousTime -= dt;
            }
            if (this.currentFixSkillContinuousTime > 0) {
                tempFixSkillTime = Math.ceil(this.currentFixSkillContinuousTime);
                this.fixTimeLabel.string = tempFixSkillTime;
                this.fixMask.active = false;
                return;
            }
        }
        if (!!this.currentFixSkillCDTime) {
            if (this.currentFixSkillCDTime > 0) {
                this.currentFixSkillCDTime -= dt;
            }
        }
        if (this.currentFixSkillCDTime > 0) {
            tempFixSkillTime = Math.ceil(this.currentFixSkillCDTime);
            this.fixTimeLabel.string = tempFixSkillTime;
            this.fixMask.active = true;

            let cdTime = this.currentFixSkillCDTime / this.currentFixSkillCDAllTime;
            this.fixMask.getComponent(cc.Sprite).fillRange = cdTime;
        } else {
            this.fixTimeLabel.string = "";
            this.fixMask.active = false;
        }
    },

    updateSpeedSkillTime: function (dt) {
        let tempSpeedSkillTime = 0;
        if (!!this.currentSpeedSkillContinuousTime) {
            if (this.currentSpeedSkillContinuousTime > 0) {
                this.currentSpeedSkillContinuousTime -= dt;
            }
            if (this.currentSpeedSkillContinuousTime > 0) {
                tempSpeedSkillTime = Math.ceil(this.currentSpeedSkillContinuousTime);
                this.speedTimeLabel.string = tempSpeedSkillTime;
                this.speedMask.active = false;
                return;
            }
        }
        if (!!this.currentSpeedSkillCDTime) {
            if (this.currentSpeedSkillCDTime > 0) {
                this.currentSpeedSkillCDTime -= dt;
            }
        }
        if (this.currentSpeedSkillCDTime > 0) {
            tempSpeedSkillTime = Math.ceil(this.currentSpeedSkillCDTime);
            this.speedTimeLabel.string = tempSpeedSkillTime;
            this.speedMask.active = true;

            let cdTime = this.currentSpeedSkillCDTime / this.currentSpeedSkillCDAllTime;
            this.speedMask.getComponent(cc.Sprite).fillRange = cdTime;
        } else {
            this.speedTimeLabel.string = "";
            this.speedMask.active = false;
        }
    },

    update: function (dt) {
        this.updateFixSkillTime(dt);
        this.updateSpeedSkillTime(dt);
        this.updateRobotSkillTime(dt);
    ...
    clickSpeedBtn: function () {
        ...
        roomAPI.gameMessageNotify(gameProto.gameUserSkillsNotify(this.selfChairID, gameProto.Skills.skillsType2));
    },

    clickFixBtn: function () {
        if (!!this.currentFixSkillContinuousTime) {
            if (this.currentFixSkillContinuousTime > 0) {
                return;
            }
        ...
        roomAPI.gameMessageNotify(gameProto.gameUserSkillsNotify(this.selfChairID, gameProto.Skills.skillsType1));
    },

    //延迟机器人释放技能
    delayRobotFireSkill: function () {
        this.scheduleOnce(function () {
            this.robotFireSkill()
        }.bind(this), 10);
    },

    //机器人释放技能 技能id，如果现在有倍数很高的鱼就极速，如果现在界面中的鱼很多就冰冻
    robotFireSkill: function () {
        if (!this.robotChairArr || this.robotChairArr.length === 0) {
            this.delayRobotFireSkill();
            return;
        }
        if (this.currentState === gameProto.FGStatus.ReadyFishArrayStatus) {
            cc.log("鱼阵准备状态下机器人不能释放技能");
            this.delayRobotFireSkill();
            return;
        }
        let skillId = 0;
        let fishLen = this.fishWidgetCtrlArr.length;
        if (fishLen > 300) {//当前已生成的鱼数量大于200释放定屏技能
            cc.log("当前屏幕生成的鱼数量超过200条，机器人使用定屏技能");
            skillId = gameProto.Skills.skillsType1;
        } else {
            let fireSpeed = false;
            for (let i = 0; i < fishLen; ++i) {
                let fishTypeInfo = this.fishWidgetCtrlArr[i].fishTypeInfo;
                if (!fishTypeInfo) {
                    continue;
                }
                if (fishTypeInfo.rewardTimes >= 30) {//如果当前有奖励倍数超过30的鱼就释放极速技能
                    fireSpeed = true;
                    break;
                }
            }
            if (fireSpeed === true) {
                cc.log("当前屏幕生成的鱼有奖励倍数超过30的，机器人使用极速技能");
                skillId = gameProto.Skills.skillsType2;
            }
        }
        if (skillId === 0) {
            cc.log("生成鱼数量不到200， 屏幕中也没有奖励倍数达到30的鱼");
            this.delayRobotFireSkill();
            return;
        }
        let robotLen = this.robotChairArr.length;
        ...
        let chairID = this.robotChairArr[randomIndex];
        // skillId = 1;
        let fireSkill = this.checkRobotSkill(chairID, skillId);
        if (fireSkill === true) {
            cc.log("座位号:" + chairID + ", 选择技能:" + skillId);
            roomAPI.gameMessageNotify(gameProto.gameUserSkillsNotify(chairID, skillId));
        }
        this.delayRobotFireSkill();
    },

    //检测机器人技能是否可以释放
    checkRobotSkill: function (chairId, skillId) {
        if (!this.robotSkillInfo) {
            return true;
        }
        let currentChairItem = null;
        let len = this.robotSkillInfo.length;
        for (let i = 0; i < len; ++i) {
            let robotSkillItem = this.robotSkillInfo[i];
            if (robotSkillItem.chairId === chairId) {
                currentChairItem = robotSkillItem;
            }
        }
        if (!!currentChairItem) {
            if (skillId === gameProto.Skills.skillsType1) {
                if (!!currentChairItem.skill1) {
                    if (currentChairItem.skill1 >= 0) {
                        cc.log("座位[" + chairId + "]技能1未冷却:" + currentChairItem.skill1);
                        return false;
                    } else {
                        return true;
                    }
            ...
            } else if (skillId === gameProto.Skills.skillsType2) {
                if (!!currentChairItem.skill2) {
                    if (currentChairItem.skill2 >= 0) {
                        cc.log("座位[" + chairId + "]技能2未冷却:" + currentChairItem.skill2);
                        return false;
                    } else {
                        return true;
                    }
    ...
    //更新机器人技能信息,释放时间，cd时间
    updateRobotSkillInfo: function (chairID, skillsType, skillsKillTime, skillsOkTime) {
        if (!this.robotChairArr || this.robotChairArr.length === 0) {
            return;
        }
        if (this.robotChairArr.indexOf(chairID) === -1) {
            return;
        }
        if (!this.robotSkillInfo) {
            this.robotSkillInfo = [];
        ...
        let len = this.robotSkillInfo.length;
        let isInsert = false;
        let tempTime = skillsKillTime + skillsOkTime;
        for (let i = 0; i < len; ++i) {
            let robotSkillItem = this.robotSkillInfo[i];
            if (robotSkillItem.chairId != chairID) {
                continue;
            ...
            isInsert = true;
            if (skillsType === gameProto.Skills.skillsType1) {
                robotSkillItem.skill1 = tempTime;
            } else if (skillsType === gameProto.Skills.skillsType2) {
                robotSkillItem.skill2 = tempTime;
            }
            break;
        }
        if (isInsert != true) {
            let newRobotSkillItem = {};
            newRobotSkillItem.chairId = chairID;
            if (skillsType === gameProto.Skills.skillsType1) {
                newRobotSkillItem.skill1 = tempTime;
            } else if (skillsType === gameProto.Skills.skillsType2) {
                newRobotSkillItem.skill2 = tempTime;
            }
            this.robotSkillInfo.push(newRobotSkillItem);
        }
    },

    //每帧更新机器人技能时间
    updateRobotSkillTime: function (dt) {
        if (!this.robotSkillInfo || this.robotSkillInfo.length === 0) {
            return;
        }
        let len = this.robotSkillInfo.length;
        for (let i = 0; i < len; ++i) {
            let robotSkillItem = this.robotSkillInfo[i];
            if (!!robotSkillItem.skill1) {
                if (robotSkillItem.skill1 <= 0) {
                    robotSkillItem.skill1 = null;
                    continue;
                }
                robotSkillItem.skill1 -= dt;
            }
            if (!!robotSkillItem.skill2) {
                if (robotSkillItem.skill2 <= 0) {
                    robotSkillItem.skill2 = null;
                    continue;
                }
                robotSkillItem.skill2 -= dt;
            }
        }
    },

    //玩家释放技能
    userSkillsPush: function (data) {
        let chairID = data.chairID;
        let skillsType = data.skillsType;
        let skillsKillTime = data.skillsKillTime;
        let skillsOkTime = data.skillsOkTime;
        if (chairID === this.selfChairID) {
            let modeHint = null;
            if (skillsType === gameProto.Skills.skillsType1) {
                this.currentFixSkillContinuousTime = skillsKillTime;
                this.currentFixSkillCDTime = skillsOkTime;
                this.currentFixSkillCDAllTime = skillsOkTime;
                modeHint = this.fixHint;
            } else if (skillsType === gameProto.Skills.skillsType2) {
                this.currentSpeedSkillContinuousTime = skillsKillTime;
                this.currentSpeedSkillCDTime = skillsOkTime;
                this.currentSpeedSkillCDAllTime = skillsOkTime;
                this.fireControlWidgetCtrl.setSpeedSkill(true);
                modeHint = this.speedHint;
            }
            modeHint.active = true;
            let hintAnimation = modeHint.getComponent(cc.Animation);
            if (!!hintAnimation) {
                hintAnimation.play();
                hintAnimation.on("stop", (event) => {
                    modeHint.active = false;
                }, this);
            }
            if (this.modeSpr.active != true) {
                this.modeSpr.active = true;
                this.modeSpr.runAction(cc.repeatForever(cc.sequence(cc.tintTo(1, 255, 0, 0), cc.tintTo(1, 255, 255, 255), cc.tintTo(1, 0, 255, 0), cc.tintTo(1, 255, 255, 255))));
            }
        }
        else {
            this.updateRobotSkillInfo(chairID, skillsType, skillsKillTime, skillsOkTime);
        }
    },
    //玩家技能冷却完成
    userSkillsOkNotify: function (data) {
        let skillsType = data.skillsType;
        if (skillsType === gameProto.Skills.skillsType1) {
            this.currentFixSkillCDTime = null;
        } else if (skillsType === gameProto.Skills.skillsType2) {
            this.currentSpeedSkillCDTime = null;
        }
    },
    //玩家技能释放完成
    userSkillsKillNotify: function (data) {
        let skillsType = data.skillsType;
        if (skillsType === gameProto.Skills.skillsType1) {
            this.currentFixSkillContinuousTime = null;
        } else if (skillsType === gameProto.Skills.skillsType2) {
            this.currentSpeedSkillContinuousTime = null;
            this.fireControlWidgetCtrl.setSpeedSkill(false);
        }
        if (this.currentSpeedSkillContinuousTime === null && this.currentFixSkillContinuousTime === null) {
            this.modeSpr.stopAllActions();
            this.modeSpr.active = false;
        }
    },
    ...
    loadFishArray2: function (fishDataArray, actionTime) {
        ...
        for (let i = 0; i < len; ++i) {
            ...
            let ctrl = fishNode.getComponent("ShenHaiBuYuFishCtrl");
