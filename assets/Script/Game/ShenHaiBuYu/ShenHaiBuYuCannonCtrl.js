...
cc.Class({
    ...
    properties: {
        ...
        lockLineWidgetCtrl: require("ShenHaiBuYuLockLineWidgetCtrl"),
    ...
    updateCannonPic: function () {
        let gunUrl = "ShenHaiBuYu/cannon/gun";
        // if (this.isSuper === true) {
        //     gunUrl = "ShenHaiBuYu/cannon/gun_super_";
        // }
        let gunIndex = 1;
        ...
        let cannonSpr = this.cannonSpr.getComponent(cc.Sprite);
    ...
    onLockFish: function (fishCtrl) {
        if (!fishCtrl || !fishCtrl.node) {
            ...
            let lockFishCtrl = this.lockFish.getComponent("ShenHaiBuYuFishCtrl");
    ...
    onBtnClick: function (event, params) {
        if (params === "addPower") {
            ...
            AudioMgr.playSound('ShenHaiBuYu/sound/effect/cannonSwitch');
    ...
    //显示转盘
    showPrize(gainGold, baseScore) {
        let rewardTimes = gainGold / baseScore / this.powerIndex;
        if (rewardTimes < 25) {
            return;
        }
        ...
        if (rewardTimes < 50) {
            ...
            AudioMgr.playSound('ShenHaiBuYu/sound/effect/bingo');
            return;
        }
        if (rewardTimes < 100) {
        ...
        if (!!aniCtrl) {
            ...
            AudioMgr.playSound('ShenHaiBuYu/sound/effect/prize1');
        }
    },
    ...
    showGoldPillar(goldValue, baseValue) {
        ...
        for (let i = 0; i < len; ++i) {
            let pillarctrl = this.pillarList[i].getComponent("ShenHaiBuYuGoldPillarCtrl");
            if (i == 0 && len == 4) {
        ...
        let pillarCtrl = pillarItem.getComponent("ShenHaiBuYuGoldPillarCtrl");
        let showTime = 0.15;
        let moveDistance = 33;
        let moveDir = 1;
