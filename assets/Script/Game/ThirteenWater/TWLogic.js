...
proto.SanTongHua = 0x001;             //三同花
proto.SanShunZi = 0x002;              //三顺子
proto.LiuDuiBan = 0x004;              //六对半
proto.YiTiaoLong = 0x008;             //一条龙
proto.ZhiZhunYiTiaoLong = 0x010;      //至尊一条龙
...
/* 至尊一条龙 */
proto.hasZhiZhunYitiaolong = function (arr) {
	if (arr.length !== 13) { return false; }
	var j;
	for (j = 1; j < arr.length; ++j) {
		if (this.getCardColor(arr[j]) != this.getCardColor(arr[j - 1])) {
			return false;
		}
	}
	var cards = this.removeCardColor(arr);
...
proto.hasGuaipai = function (arr) {
	if (this.hasZhiZhunYitiaolong(arr)) {
		return this.ZhiZhunYiTiaoLong;
	}
	else if (this.hasYitiaolong(arr)) {
		return this.YiTiaoLong;
	}
	else if (this.hasLiuduiban(arr)) {
		return this.LiuDuiBan;
	}
	else if (this.hasSanshun(arr)) {
		return this.SanShunZi;
	}
	else if (this.hasSanhua(arr)) {
		return this.SanTongHua;
	}
	return 0;
};
...
proto.getAutoSortCardsArr = function (arr) {
	let groupCards = [];
	let chooseTypeArr = {};
	chooseTypeArr[this.cardType.tonghuashun.type] = {};
	chooseTypeArr[this.cardType.tonghuashun.type].count = 0;
	chooseTypeArr[this.cardType.sitiao.type] = {};
	chooseTypeArr[this.cardType.sitiao.type].count = 0;
	chooseTypeArr[this.cardType.hulu.type] = {};
	chooseTypeArr[this.cardType.hulu.type].count = 0;
	chooseTypeArr[this.cardType.tonghua.type] = {};
	chooseTypeArr[this.cardType.tonghua.type].count = 0;
	chooseTypeArr[this.cardType.shunzi.type] = {};
	chooseTypeArr[this.cardType.shunzi.type].count = 0;
	chooseTypeArr[this.cardType.santiao.type] = {};
	chooseTypeArr[this.cardType.santiao.type].count = 0;
	chooseTypeArr[this.cardType.liangdui.type] = {};
	chooseTypeArr[this.cardType.liangdui.type].count = 0;
	chooseTypeArr[this.cardType.duizi.type] = {};
	chooseTypeArr[this.cardType.duizi.type].count = 0;

	var chooseOneGroupCards = function (arr) {
		let tmpCards = {};
		let toudaoArr = null;
		let zhongdaoArr = null;
		let weidaoArr = null;
		let restArr = [];
		...
		let isChoosed = false;
		let resout = [];
		var chooseCards = function (cb_, type_, restArr_) {
			if (isChoosed == false) {
				resout = cb_(restArr_);
				if (resout.length > 0 && chooseTypeArr[type_].count < resout.length) {
					if (weidaoArr == null) {
						weidaoArr = resout[chooseTypeArr[type_].count];
						chooseArr = weidaoArr;
					}
					else if (zhongdaoArr == null) {
						zhongdaoArr = resout[chooseTypeArr[type_].count];
						chooseArr = zhongdaoArr;
					}
					chooseTypeArr[type_].count += 1;
					isChoosed = true;
				}
			}
		}
		for (let i = 0; i < 3; i++) {
			restArr = this.getNotContainArray(restArr, chooseArr);
			resout = [];
			chooseArr = [];
			...
			if (i == 2) {
				if (restArr.length <= 3)
					...
					toudaoArr = [restArr[0], restArr[1], restArr[2]];
					zhongdaoArr = [restArr[3], restArr[4], restArr[5], restArr[6], restArr[7]];
				}

				break;
			}

			chooseCards(this.getTonghuashun.bind(this), this.cardType.tonghuashun.type, restArr);
			chooseCards(this.getSitiao.bind(this), this.cardType.sitiao.type, restArr);
			chooseCards(this.getHulu.bind(this), this.cardType.hulu.type, restArr);
			chooseCards(this.getTonghua.bind(this), this.cardType.tonghua.type, restArr);
			chooseCards(this.getShunzi.bind(this), this.cardType.shunzi.type, restArr);
			chooseCards(this.getSantiao.bind(this), this.cardType.santiao.type, restArr);
			chooseCards(this.getLiangdui.bind(this), this.cardType.liangdui.type, restArr);
			chooseCards(this.getDuizi.bind(this), this.cardType.duizi.type, restArr);

			if (i == 0 && isChoosed == false)
				return null;
		}
		tmpCards.toudaoArr = toudaoArr;
		// tmpCards.toudaoCardName = "danpai"
		tmpCards.toudaoCardName = this.getTouZhongWeiCardTypeName(toudaoArr);
		tmpCards.zhongdaoArr = zhongdaoArr;
		tmpCards.zhongdaoCardName = this.getTouZhongWeiCardTypeName(zhongdaoArr);
		tmpCards.weidaoArr = weidaoArr;
		tmpCards.weidaoCardName = this.getTouZhongWeiCardTypeName(weidaoArr);
		// 对比 三道 牌型是否合法
		if (this.compareCards(zhongdaoArr, toudaoArr,
			this.getTouZhongWeiCardType(zhongdaoArr),
			this.getTouZhongWeiCardType(toudaoArr)) &&
			this.compareCards(weidaoArr, zhongdaoArr,
				this.getTouZhongWeiCardType(weidaoArr),
				this.getTouZhongWeiCardType(zhongdaoArr))) {
			...
			console.error("排除一个错误的自动摆牌数据::", tmpCards)
		}
		return null;
	...
	for (let i = 0; i < 10; i++) {
		let oneGroupCards = chooseOneGroupCards(arr);
		if (oneGroupCards != null) {
			groupCards.push(oneGroupCards);
	...
	for (let i = 0; i < groupCards.length; i++) {
		let type = groupCards[i].toudaoCardName + groupCards[i].zhongdaoCardName + groupCards[i].weidaoCardName;
		if (!obj[type]) {
			obj[type] = groupCards[i];
		}
	}
	let retGroups = []
	for (const key in obj) {
		retGroups.push(obj[key]);
	}
	obj = null;
	// return groupCards;
	return retGroups;
...
proto.getTouZhongWeiCardTypeName = function (arr) {
	var typeName = "danpai";
	if (this.hasDuizi(arr)) { typeName = this.cardType.duizi.name; }
	if (this.hasLiangdui(arr)) { typeName = this.cardType.liangdui.name; }
	if (this.hasSantiao(arr)) { typeName = this.cardType.santiao.name; }
	if (this.hasShunzi(arr)) { typeName = this.cardType.shunzi.name; }
	if (this.hasTonghua(arr)) { typeName = this.cardType.tonghua.name; }
	if (this.hasHulu(arr)) { typeName = this.cardType.hulu.name; }
	if (this.hasSitiao(arr)) { typeName = this.cardType.sitiao.name; }
	if (this.hasTonghuashun(arr)) { typeName = this.cardType.tonghuashun.name; }
	return typeName;
};
...
proto.sortCardByColor = function (arr) {
	var cards = [];
	let tmpArr1 = [], tmpArr2 = [], tmpArr3 = [], tmpArr4 = [];
	var i;
	for (i = 0; i < arr.length; ++i) {
		if (proto.getCardColor(arr[i]) == "fangkuai") {
			tmpArr1.push(arr[i]);
		}
		else if (proto.getCardColor(arr[i]) == "meihua") {
			tmpArr2.push(arr[i]);
		}
		else if (proto.getCardColor(arr[i]) == "hongtao") {
			tmpArr3.push(arr[i]);
		}
		else if (proto.getCardColor(arr[i]) == "heitao") {
			tmpArr4.push(arr[i]);
		}
	}

	tmpArr1.sort(function (a, b) { return b - a; });
	tmpArr2.sort(function (a, b) { return b - a; });
	tmpArr3.sort(function (a, b) { return b - a; });
	tmpArr4.sort(function (a, b) { return b - a; });

	for (let i = 0; i < tmpArr1.length; i++) {
		const item = tmpArr1[i];
		cards.push(item);
	}
	for (let i = 0; i < tmpArr2.length; i++) {
		const item = tmpArr2[i];
		cards.push(item);
	}
	for (let i = 0; i < tmpArr3.length; i++) {
		const item = tmpArr3[i];
		cards.push(item);
	}
	for (let i = 0; i < tmpArr4.length; i++) {
		const item = tmpArr4[i];
		cards.push(item);
	}
	...
	for (i = 0; i < memberCount; ++i) {
		...
		for (j = 0; j < memberCount; ++j) {
			...
			for (k = 0; k < 3; ++k) {
				...
				scoreArr_single[i] += resout.scoresArr[i][j][k];
			}
		...
		scoreArr[i] -= scoreArr_single[i];


		if(resout.guaipaiScoreArr[i] != null){
			let length = resout.guaipaiScoreArr[i].length;
			for(let j1 = 0;j1<length;j1++){
				guaiScore += resout.guaipaiScoreArr[i][j1];
			}
		}
		scoreArr[i] += guaiScore;
		//scoreArr_single[i] += guaiScore;
	}

	score.scoreArr = scoreArr;
	score.scoreArr_single = scoreArr_single;

	return score;
};

proto.getResultScore = function(resout,curIndex,baseScore){
	let scoreByResout = this.getScoreArrByResout(resout);
	let score = scoreByResout.scoreArr[curIndex];
	let score_single = scoreByResout.scoreArr_single[curIndex];
	let resultScore = (score + score_single) * baseScore;
...
proto.getTouScoreArrByResout = function (resout) {
	...
	for (i = 0; i < memberCount; ++i) {
		for (j = 0; j < memberCount; ++j) {
			...
			scoreArr_single[i] += resout.scoresArr[i][j][0];
		}
...
proto.getZhongScoreArrByResout = function (resout) {
	...
	for (i = 0; i < memberCount; ++i) {
		for (j = 0; j < memberCount; ++j) {
			...
			scoreArr_single[i] += resout.scoresArr[i][j][1];
		}
...
proto.getWeiScoreArrByResout = function (resout) {
	...
	for (i = 0; i < memberCount; ++i) {
		for (j = 0; j < memberCount; ++j) {
			...
			scoreArr_single[i] += resout.scoresArr[i][j][2];
		}
...
proto.sortSpecialCard = function (arr) {
	if (this.hasYitiaolong(arr) || this.hasZhiZhunYitiaolong()) {
